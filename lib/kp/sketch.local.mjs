...
console.log(`Расстановка: рядов ${s.rows.length} · зон ${s.zones.length} · режим ${s.mode ?? '—'}` +
  ` · шаг ${s.sectionWidth ?? '—'} · глубина ряда ${s.rowDepth ?? '—'} · проход ${s.aisle ?? '—'}`);
for (const r of s.rows) {
  console.log(`  — ${r.name ?? 'без подписи'}: ${(r.length / 1000).toFixed(1)} м по ${r.axis}` +
    `${r.depth ? `, глубина ${r.depth} мм${r.double ? ' (спаренный)' : ''}` : ''}` +
    `${r.sections.length ? `, секции: ${r.sections.join('/')}` : ''}`);
}
for (const z of s.zones) console.log(`  ▨ ${z.name ?? z.kind} (${z.kind})`);
