...
export default function BlogLibrary({ articles, lang = 'ru' }) {
  const L = lang === 'uz' ? 'uz' : 'ru';
  const ui = BLOG_UI[L];
  const allLabel = ui.all;
  const categories = useMemo(() => [allLabel, ...Array.from(new Set(articles.map((a) => a.category)))], [articles, allLabel]);
  const [cat, setCat] = useState(allLabel);
  const list = cat === allLabel ? articles : articles.filter((a) => a.category === cat);

  const fmt = (d) => {
    const [y, m, day] = d.split('-');
    return `${parseInt(day, 10)} ${MONTHS[L][parseInt(m, 10) - 1]} ${y}`;
  };
          ...
          <a key={a.slug} href={`/blog/${a.slug}`} className="group block h-full rounded-xl2 overflow-hidden bg-white border border-cloud-200 shadow-card hover:shadow-card-hover hover:-translate-y-1 transition">
            ...
            <div className="p-5 flex flex-col">
              <div className="flex items-center gap-2 text-xs text-slate-400">
                ...
                <span className="inline-flex items-center gap-1"><IcoClock className="w-3.5 h-3.5" /> {a.readMins} {ui.min}</span>
              </div>
              ...
              <span className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-navy-700 group-hover:text-sky-600">{ui.readShort} <IcoArrow className="w-4 h-4" /></span>
            </div>
          </a>
        ))}
      </div>
