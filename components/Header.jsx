...
export default function Header({ lang = 'ru' }) {
  ...
  const LangToggle = ({ className = '' }) => (
    <div className={`flex items-center rounded-lg border border-white/20 overflow-hidden text-sm ${className}`}>
      {['ru', 'uz'].map((l) => (
        <button
          key={l}
    ...
    <header className="fixed top-0 inset-x-0 z-50">
      <div className="w-full px-4 sm:px-6 lg:px-10 mt-3">
        <div className={`relative flex items-center gap-4 h-16 rounded-2xl px-4 sm:px-5 border transition-all ${scrolled ? 'bg-navy-900/90 backdrop-blur-md border-white/10 shadow-band' : 'bg-white/10 backdrop-blur-md border-white/15'}`}>
          <a href="/" className="flex items-center shrink-0" aria-label="RAXPRO">
          ...
          <div className="flex items-center gap-2.5 ml-auto xl:ml-0 shrink-0">
            ...
            <a href="#zayavka" className="hidden lg:inline-flex text-sm font-medium px-5 py-2.5 rounded-xl bg-white/12 border border-white/20 text-white hover:bg-white hover:text-navy-800 transition backdrop-blur-sm whitespace-nowrap">
              {tr.consult}
            </a>
            <button onClick={() => setOpen((v) => !v)} className="xl:hidden w-10 h-10 grid place-items-center rounded-xl bg-white/12 border border-white/15 text-white" aria-label="Menu">
        ...
        {open && (
          <div className="xl:hidden mt-2 rounded-2xl bg-navy-900/95 backdrop-blur-md border border-white/10 px-4 py-4">
            ...
            <div className="mt-3 flex items-center justify-between gap-2">
              <a href={`tel:${SITE.phoneMain}`} className="flex items-center gap-2 font-medium text-white">
                <IcoPhone className="w-4 h-4 text-sky-300" /> {SITE.phoneMainHuman}
              </a>
              <LangToggle />
            </div>
            <a href="#zayavka" onClick={() => setOpen(false)} className="mt-3 block bg-brand-grad text-white text-center font-semibold px-5 py-3 rounded-xl">{tr.consult}</a>
          </div>
        )}
      </div>
    </header>
